...
from .routers import subscriptions, users, vpn, admin
from .loops import disable_expired_vpn_peers_loop, monitor_and_fix_wg0_loop, monitor_health_loop, notify_expiring_soon_loop, notify_expiring_subscriptions_24h_loop, admin_daily_digest_loop
...
async def lifespan(app: FastAPI):
    ...
    task_notify_expiring_24h = asyncio.create_task(notify_expiring_subscriptions_24h_loop())
    task_monitor_wg = asyncio.create_task(monitor_and_fix_wg0_loop())
    task_monitor_health = asyncio.create_task(monitor_health_loop())
    task_admin_digest = asyncio.create_task(admin_daily_digest_loop())
    
    yield
    # Останавливаем задачи при завершении приложения
    task_disable_expired.cancel()
    task_notify_expiring.cancel()
    task_notify_expiring_24h.cancel()
    task_monitor_wg.cancel()
    task_monitor_health.cancel()
    task_admin_digest.cancel()
    try:
    ...
    try:
        ...
        pass
    try:
        await task_notify_expiring_24h
    except asyncio.CancelledError:
        pass
    try:
